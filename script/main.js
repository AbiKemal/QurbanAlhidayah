import config from "./config.js";

google.charts.load("current", { packages: ["table"] });
google.charts.setOnLoadCallback(loadPesertaList);

function loadPesertaList() {
  const sheetName = config.sheets.PESERTA_QURBAN.sheet_name;
  const queryString = encodeURIComponent("SELECT A, B");

  const url = `https://docs.google.com/spreadsheets/d/${config.gsheet}/gviz/tq?sheet=${sheetName}&headers=1&tq=${queryString}`;

  const query = new google.visualization.Query(url);
  query.send(function (response) {
    if (response.isError()) {
      document.getElementById("peserta_list").innerHTML =
        "Gagal memuat data peserta.";
      return;
    }

    const data = response.getDataTable();
    let html = "<ul>";
    for (let i = 0; i < data.getNumberOfRows(); i++) {
      const nama = data.getValue(i, 1);
      html += `<li><a href="view.html?nama=${encodeURIComponent(
        nama
      )}">${data.getValue(i, 0)} - ${nama}</a></li>`;
    }
    html += "</ul>";
    document.getElementById("peserta_list").innerHTML = html;
  });
}
